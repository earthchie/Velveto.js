...
var Velveto = new function(){
	...
	var load = function(filename,history){
		...
		setTimeout(function(){ // wait 10ms, to make sure all active ajax is aborted.
			...
			if(body){
				...
				fireEvent(document.getElementById('velveto'),'beforeLoad');
				...
				ajax.get('app/'+filename+'.html',function(r){
					...
					fireEvent(document.getElementById('velveto'),'loaded');
					...
					fireEvent(document.getElementById('velveto'),'loadFail');
				});
			}else{
	 ...
	 * return: void
	 */
	this.goTo = function(path,history,updatePath){
	...
	this.refresh = function(){
		this.goTo(window.location.hash,false,false);
	}
