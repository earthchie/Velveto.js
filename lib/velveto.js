...
(function (window) {
    ...
    Velveto.session = {
        get: function (session) {
            if (localStorage) {
                return localStorage.getItem(session) || Velveto.tmp[session];
            } else {
    ...
    Velveto.data = function (path) {
        var params = Velveto.getPathData(path).params,
            data = Velveto.getPathData(path).dir,
            i;
    ...
    Velveto.scrollTo = function (to, duration) {
        if (typeof to === 'object' && typeof to.offsetTop === 'number') {
