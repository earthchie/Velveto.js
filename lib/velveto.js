...
(function (window) {
    ...
    Velveto.ajax = {
        ...
        executeScript: function (url, callback) {
            ...
            script.setAttribute('data-velveto-script', '');
    ...
    Velveto.importDocument = function (url, container, callback) {
        ...
        var loading = container.getAttribute('data-loading-indicator') || Velveto.options.loading_indicator,
            page_scripts = document.querySelectorAll('[data-velveto-script]'),
            i;
        
        for (i = 0; i < page_scripts.length; i = i + 1) {
            page_scripts[i].parentNode.removeChild(page_scripts[i]);
        }
            ...
            if (scripts.length > 0) {
                ...
                executeScript = function (script) {
                    ...
                    if (typeof script !== 'undefined') {
                        ...
                        if (script.innerHTML) { // alternative eval (hack)
                            image = document.createElement('img');
                            image.setAttribute('onload', 'this.parentNode.removeChild(this);' + script.innerHTML);
